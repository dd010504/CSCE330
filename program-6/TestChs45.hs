...
scoreHalve :: Int
scoreHalve = if test1 && test2  then 5  else 0
    where
        test1 = halve [1..10] == ([1..5],[6..10])
        test2 = halve ['a'..'z'] == (['a'..'m'],['n'..'z'])

scoreThird :: Int
scoreThird = if allPass  then 3  else 0
    where
        test1 = third [1..10] == 3
...
scoreGrid :: Int
scoreGrid = if allPass then 5 else 0
    where
        grid1 = sort $ grid 1 2
...
scoreSquare :: Int
scoreSquare = if allPass then 3 else 0
    where
        sq1 = sort $ square 1 
        ans1 = sort [(0,1),(1,0)]
        sq2 = sort $ square 2
...
scoreRepl :: Int
scoreRepl = if allPass then 3 else 0
    where
